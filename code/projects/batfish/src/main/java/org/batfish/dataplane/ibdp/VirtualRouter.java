...
public final class VirtualRouter {
  ...
  public ArrayList<Long> _logRuntime;
  ...
  public static boolean IsNewSraticRoute = false;
  ...
  VirtualRouter(@Nonnull String name, @Nonnull Node node) {
    ...
    //add log
    _logs = new BgpLogs(_c.getHostname()); // BgpLogs
    _logRuntime =new ArrayList<Long>();
    _name = name;
    ...
    if (_vrf.getBgpProcess() != null) {
      _bgpRoutingProcess =
          new BgpRoutingProcess(
              _vrf.getBgpProcess(), _c, _name, _mainRib, BgpTopology.EMPTY, _prefixTracer, this._logs);
    }
  }
  ...
  void bgpIteration(Map<String, Node> allNodes, int logIterNum) {
    if (_bgpRoutingProcess == null) {
      return;
    }
    //log
    BgpLog log = new BgpLog(logIterNum);
    _logs.addLog(log);
  ...
  void redistribute() {
    //第一轮添加重分发检查
    if(IncrementalBdpEngine.iterationFlag){
      //检查该节点的每条流的redistribution是否为true
      String host = _c.getHostname();
      boolean redist = false;
      // 存储当前节点的所有condition
      List<BgpCondition> multiFlowConditions = new ArrayList<>();
      BgpConditionsMap.BgpConditions.forEach((ipStr, conditions) -> {
        if(conditions.containsKey(host)){
          multiFlowConditions.add(conditions.get(host));
        }
      }
    );
//      if(BgpConditionsMap.BgpConditions.containsKey(host)){
//        condition = BgpConditionsMap.BgpConditions.get(host);
//        redist = condition.getRedistribution();
//      }
      //redistribution为false不做处理，为true特殊处理
      //记录_mainRibDeltaPrevRound里的所有路由，用于后续判断需要重分发的前缀是否已经存在
      List<AbstractRoute> mainRoute = new ArrayList<>();
      boolean existFlag = false;  //标志mainRib中是否含有该前缀
      _mainRibDeltaPrevRound.getActions().forEach(a -> mainRoute.add(a.getRoute().getRoute()));
      for(BgpCondition c : multiFlowConditions) {
        if (c.getRedistribution()) {
          String ipPrefix = c.get_networkString();
          System.out.println("******************************redistribute**************************************");
          System.out.println(ipPrefix);
          for (AbstractRoute r : mainRoute) {
            if (r.getNetwork().toString().equals(ipPrefix)) {
              existFlag = true;
              System.out.println(r);
              break;
            }
          }
          //TODO：如果不存在该路由,new静态路由,记录违规信息
          if (!existFlag) {
            //TODO:设置标志位
            IsNewSraticRoute = true;
            //TODO:生成新的静态路由(下一跳为接口）
            String srIp = ipPrefix.split("/")[0];
            int len = Integer.parseInt(ipPrefix.split("/")[1]);
            String nextHop = _c.getActiveInterfaces(_name).values().toArray()[0].toString();
            nextHop = nextHop.split("<")[1].split(">")[0];
            StaticRoute new_sr = StaticRoute.builder()
                .setNextHopInterface(nextHop)
                .setNetwork(Prefix.create(Ip.parse(srIp), len))
            ...
            violation.get(host).setViolateRedis("Invalid route to redistribute!");
            //因为创建的路由都为静态路由，故不做路由类型判断
            violation.get(host).setOriginStaticRoute(new_sr);

            //System.out.println(ViolationsMap.violationsMap);

            //TODO：将静态路由导入到mainrib里
            RibDelta<AnnotatedRoute<AbstractRoute>> deltaRIB = RibDelta.<AnnotatedRoute<AbstractRoute>>builder()
                .add(annotateRoute(new_sr))
                .build();
            _mainRibDeltaPrevRound = RibDelta.merge(deltaRIB, _mainRibDeltaPrevRound);
            //将新建的静态路由添加到静态路由表里
            _staticUnconditionalRib.mergeRouteGetDelta(new_sr);
            importRibDelta(_mainRib, deltaRIB);
            //测试生成的路由是否成功导入
            //          System.out.println("***************************************");
            //          System.out.println(_mainRibDeltaPrevRound);
            //          System.out.println("***************************************");
          }
        }
      }
  ...
  void mergeBgpRoutesToMainRib() {
    ...
    //    System.out.println("HostName: "+_bgpRoutingProcess.get_hostname());
    RibDelta<AnnotatedRoute<AbstractRoute>> mainRibDelta = importRibDelta(_mainRib, _bgpRoutingProcess.getUpdatesForMainRib(), _name);

    _mainRibRouteDeltaBuilder.from(mainRibDelta);

    BgpLog log = _logs.get_logs().get(_logs.get_logs().size()-1);

    if (!mainRibDelta.isEmpty()){
      mainRibDelta
          .getActions()
          .forEach(a -> {
            long startTime = System.nanoTime();
            log.addInstalled(a.getRoute().getRoute());
            long endTime = System.nanoTime();
            _logs.runTimeAdd(endTime,startTime);
          });
    }
    //    for (String key:log._bgpReceived.keySet()){
    //      for (AbstractRoute r:log._bgpReceived.get(key)){
    //
    //        if (_mainRib.containsRoute(annotateRoute(r))){
    ////          long startTime = System.nanoTime();
    //          log._installed.add(r);
    //
    //        }
    //      }
    //    }
  ...
  public void recordOriginRoute(){
    BgpLog log_ier0 = new BgpLog(0);
    _logs.get_logs().add(log_ier0);
    for (AbstractRoute r:_mainRib.getRoutes()){
      long startTime = System.nanoTime();
      log_ier0.addInstalled(r);
      long endTime = System.nanoTime();
      _logs.runTimeAdd(endTime, startTime);
  ...
  // yrl-add
  public BgpRoutingProcess getBgpRoutingProcessYrl() {
    return _bgpRoutingProcess;
  }
