...
public abstract class BgpRib<R extends BgpRoute<?, ?>> extends AbstractRib<R> {
  ...
  /* Indicates whether current BGP route should be preferred. */
  private boolean IsSelectedRoute = false;
  /* Indicates whether selected route has been import. */
  private boolean HasSelectedRoute = false;
  /* Indicates whether routing priority has changed. 0: No change; 1: selected route replace current bestroute; 2: current route better than selected route but no replace*/
  private Integer PreferModify = 0;

  private R recordRoute;
  ...
  public int comparePreference(R lhs, R rhs) {
    ...
    if (multipathCompare != 0 || isMultipath()) {
      //当前的watpoint属性都是通过set lp实现的，因此暂时只考虑上面的这些比较条件
      if(Client.symbolic){
        //需要被优选，但未优选
        if(IsSelectedRoute && multipathCompare < 0){
          //让当前路由被优选
          multipathCompare = 1;
          //设置标志位，标记需要记录违规信息
          PreferModify = 1;
        ...
        //不是被优选的路由，但实际被优选，主要为了确保被优选的路由不会被后面收到的路由replace，导致路由震荡
        //但有可能这条路由并不是req要求的前缀，需要在记录时做判断
        else if(HasSelectedRoute && !IsSelectedRoute && multipathCompare > 0){
          //让要求的route保持被优选
          multipathCompare = -1;
          //设置标志位，标记需要记录违规信息
          PreferModify = 2;
  ...
  public boolean getIsSelectedRoute() {return IsSelectedRoute;}

  public boolean getHasSelectedRoute() { return HasSelectedRoute;}

  public Integer getPreferModify(){ return PreferModify; }

  public void setIsSelectedRoute(boolean flag) { IsSelectedRoute = flag;}

  public void setHasSelectedRoute(boolean flag) { HasSelectedRoute = flag;}

  public void setPreferModify(Integer flag) { PreferModify = flag;}

  public R getRecordRoute() {return recordRoute;}
}
