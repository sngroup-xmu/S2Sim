...
public class Client extends AbstractClient implements IClient {
  ...
  public static boolean symbolic = false;
  ...
  private void logOutput(FileWriter outWriter, String message) {
    if (outWriter == null) {
      ...
      try {
        ...
        throw new BatfishException("Failed to org.batfish.log output to outWriter", e);
      }
    }
  }
  ...
  private boolean pollWorkAndGetAnswer(WorkItem wItem, @Nullable FileWriter outWriter) {
    ...
    // get the JSON org.batfish.log of the work performed
    String answerString =
  ...
  private boolean processCommand(
    ...
    for (int i=0; i<words.length; i++) {
      if (words[i].contains("symbolic")) {
        Client.symbolic = true;
      }
      if (words[i].contains("nosymbolic")) {
        Client.symbolic = false;
      }
    }
    if (Client.symbolic) {
      String[] wordsBackup = new String[words.length-1];
      int j = 0;
      for (int i=0; i<words.length; i++) {
        if (!words[i].contains("symbolic")) {
          wordsBackup[j] = words[i];
        }
        j++;
      }
      words = wordsBackup;
    ...
    switch (command) {
      ...
      case INIT_SNAPSHOT:
        networkName = words[words.length-2];
        caseType = words[words.length-1];
        configRootPath = words[1];
        if (parameters.size()>2) {
          parameters = (new ArrayList<String>(parameters)).subList(0, parameters.size()-2);
        }
  ...
  private boolean showBatfishLogLevel(List<String> options, List<String> parameters) {
    ...
    _logger.outputf("Current batfish org.batfish.log level is %s\n", _settings.getBatfishLogLevel());
    return true;
  }
  ...
  private boolean showLogLevel(List<String> options, List<String> parameters) {
    ...
    _logger.outputf("Current client org.batfish.log level is %s\n", _logger.getLogLevelStr());
    return true;
  }
